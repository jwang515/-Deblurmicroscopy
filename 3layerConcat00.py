...
def mod():
    ...
    layer01,layer02 = unet0(input0)
    output0in = Conv2D(2, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(layer02))
    
    inputA0 = tf.keras.layers.concatenate([inputA,output0in],
                            axis=3)
    #unet for A
    
    layer1,layer2,outputA = unetA(inputA0,layer01)
    outputAin = Conv2D(2, 2, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(UpSampling2D(size = (2,2))(outputA))
    #inputB = tf.concat((inputB,outputAin),3)
    
    outputB = tf.keras.layers.concatenate([inputB,outputAin],
                            axis=3)
    #unet for B
    outputB = unetB(layer1,layer2,inputB)

    
    #model = tf.keras.Model(inputs = inputA, outputs = outputA)
    
    model = Model(inputs = [input0,inputA,inputB],outputs = [layer02,outputA,outputB])
...
def unetA(X,layer01):
    ...
    return conv5,conv6,conv8
    
def unetB(layer1,layer2,X): 
...
saveAddr = "C:/Users/vwang/OneDrive/Desktop/model/"
